...
# ==========    Tasks running when game start
    ...
    def hitCheckTask(self, task):
        for ball in self.fireballs.getFireballsList():
    ...
    def playerHealthCheckTask(self, task):
        ...
        if height < 5 or self.health < 0:
            ...
            if self.level is 1:
                vec = self.player.getPosition() - PLAYER_POSITIONS[2]
                if vec.length() < 3:
                    print "Completed level 1"
                    self.level = 2
                    self.completeLevelSound.play()
            else:
                vec = self.player.getPosition() - PLAYER_POSITIONS[3]
                if vec.length() < 3:
    ...
    def attackingTask(self, task):
        for shield in self.shields:
            ...
            # check if the shield is away from home
            ...
            if pushed:
                ...
                vector.normalize()
                self.player.movement(vector * SHILED_PUSHING_POWER)

            if tooFarFromHome:
                shield.goBackHome()

            if (not targetWithinRange) and (not awayFromHome):
